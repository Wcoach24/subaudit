...
function SubscriptionCard({ sub }: { sub: DetectedSubscription }) {
  ...
  return (
    <div className={`border border-[#1a202c] rounded-lg overflow-hidden transition-colors hover:border-[#1a202c]/80 ${colors.bg}`}>
      <div
        className="p-4 cursor-pointer hover:bg-[#12161e]/50 transition-colors"
      ...
      >
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            {sub.faviconUrl ? (
              ...
              <div className="w-8 h-8 rounded flex-shrink-0 bg-[#12161e] flex items-center justify-center text-xs font-bold text-gray-400">
                {sub.merchantName.charAt(0)}
              </div>
            )}
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-white truncate">{sub.merchantName}</p>
            </div>
          </div>

          <div className="flex items-center gap-3 flex-shrink-0">
            <div className="text-right">
              <p className="font-jetbrains-mono font-semibold text-[#00d2ff] whitespace-nowrap">
                {formatEuro(sub.amount)}
              </p>
              <p className="text-xs text-gray-400 whitespace-nowrap">{frequencyLabel[sub.frequency] || '/mes'}</p>
            </div>
            {isExpanded ? (
        ...
        <div className="mt-3 flex items-center gap-2 flex-wrap">
          ...
          <span className="text-xs text-gray-400">
            {sub.occurrences} cargo{sub.occurrences !== 1 ? 's' : ''}
          </span>
          <span className="text-xs text-gray-400">
            {"\u00B7 desde "}{sub.firstSeen.toLocaleDateString('es-ES')}
          </span>
        </div>
      </div>
        ...
        <div className="border-t border-[#1a202c] p-4 bg-[#0b0e14]/50 space-y-4">
          <div>
            <h4 className="text-sm font-semibold text-white mb-3 flex items-center gap-2">
              <AlertTriangle size={16} className="text-[#e67e22]" />
              {"C\u00F3mo cancelar"}
            </h4>
            <ol className="space-y-2">
              {sub.cancelSteps.map((step, idx) => (
                <li key={idx} className="text-sm text-gray-300 flex gap-3">
                  <span className="font-jetbrains-mono font-semibold text-[#00d2ff] flex-shrink-0">
                    {idx + 1}.
                  </span>
                  <span>{step}</span>
                </li>
          ...
          {sub.cancelUrl && (
            <a
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#00d2ff]/10 border border-[#00d2ff]/30 text-[#00d2ff] hover:bg-[#00d2ff]/20 transition-colors text-sm font-medium"
            >
              {"Ir a p\u00E1gina de cancelaci\u00F3n"}
      ...
      {isExpanded && sub.cancelSteps.length === 0 && (
        <div className="border-t border-[#1a202c] p-4 bg-[#0b0e14]/50">
          <p className="text-sm text-gray-400">
            {"No tenemos instrucciones de cancelaci\u00F3n para este servicio. Busca en la configuraci\u00F3n de tu cuenta."}
          </p>
        </div>
      )}
    </div>
    ...
    <div className="w-full max-w-4xl mx-auto space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-6 rounded-lg bg-[#12161e] border border-[#1a202c] hover:border-[#00d2ff]/30 transition-colors">
          ...
          <div className="font-jetbrains-mono text-3xl font-bold text-[#00d2ff]">
            <AnimatedNumber value={result.totalMonthly} />
          </div>
        ...
        <div className="p-6 rounded-lg bg-[#12161e] border border-[#1a202c] hover:border-[#00d2ff]/30 transition-colors">
          ...
          <div className="font-jetbrains-mono text-3xl font-bold text-[#e67e22]">
            <AnimatedNumber value={result.totalAnnual} />
          </div>
        ...
        <div className="p-6 rounded-lg bg-[#12161e] border border-[#1a202c] hover:border-[#00d2ff]/30 transition-colors">
          ...
          <div className="font-jetbrains-mono text-3xl font-bold text-[#2ecc71]">
      ...
      <div className="p-4 rounded-lg bg-[#e67e22]/10 border border-[#e67e22]/30 flex items-start gap-3">
        ...
        <div>
          <p className="font-semibold text-white">
            {"Podr\u00EDas estar gastando "}
            <span className="font-jetbrains-mono text-[#e67e22]">{formatEuro(result.totalAnnual)}</span>
            {"/a\u00F1o en suscripciones"}
          </p>
          <p className="text-sm text-gray-300 mt-1">
            Revisa cada una y cancela las que ya no uses.
          </p>
        </div>
      </div>
      ...
      <div className="p-6 rounded-lg bg-[#00d2ff]/5 border border-[#00d2ff]/20 text-center">
        <h3 className="text-lg font-semibold text-white mb-2">
          {"\u00BFQuieres una auditor\u00EDa financiera completa?"}
        </h3>
        <p className="text-gray-400 mb-4">
          Un experto analiza todos tus gastos y te da un plan de ahorro personalizado.
        </p>
        <a
          href="#"
          className="inline-flex items-center px-6 py-2 rounded-lg bg-[#00d2ff] text-[#04060a] font-semibold hover:bg-[#00b8e6] transition-colors"
        >
          {"Solicitar auditor\u00EDa \u2014 149\u20AC"}
        </a>
      </div>
      ...
      <button
        onClick={onReset}
        className="w-full px-6 py-3 rounded-lg bg-[#12161e] border border-[#1a202c] text-white font-medium hover:border-[#00d2ff]/50 transition-colors"
      >
