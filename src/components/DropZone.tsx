...
export default function DropZone({ onFileProcessed, isProcessing, setIsProcessing }: DropZoneProps) {
  ...
  const handleFileInput = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.currentTarget.files && e.currentTarget.files.length > 0) processFile(e.currentTarget.files[0]);
  }, [processFile]);
  ...
  return (
    <div className="w-full max-w-2xl mx-auto">
      <div
        ...
        className={`relative min-h-[200px] rounded-2xl border-2 border-dashed flex flex-col items-center justify-center gap-4 p-8 transition-all duration-200 cursor-pointer ${isDragging ? 'border-[#00d2ff] bg-[#00d2ff]/5 shadow-lg shadow-[#00d2ff]/20' : 'border-[#1a202c] bg-[#0b0e14] hover:border-[#00d2ff]/50'} ${isProcessing ? 'opacity-75 pointer-events-none' : ''}`}
      >
        ...
        {isProcessing && (
          <div className="mt-6 w-full max-w-xs space-y-2">
            <div className="h-1 bg-[#1a202c] rounded-full overflow-hidden">
              <div className="h-full bg-gradient-to-r from-[#00d2ff] to-[#0099cc] transition-all duration-300" style={{ width: `${progress}%` }} />
            </div>
        ...
        <input ref={fileInputRef} type="file" accept=".csv,.xls,.xlsx" onChange={handleFileInput} className="hidden" disabled={isProcessing} />
      </div>
    </div>
  );
}
